...
use divpool::DiversifiedPoolContract;

use divpool::staking_pool::*;
// Load contracts' bytes.
near_sdk_sim::lazy_static! {
  static ref WASM_BYTES: &'static [u8] = include_bytes!("../res/divpool.wasm").as_ref();
  static ref STAKING_POOL_BYTES: &'static [u8] = include_bytes!("../res/no_wait_staking_pool.wasm").as_ref();
}
...
fn init_simulator_and_contract(
    ...
    let key = Base58PublicKey::try_from(owner.signer.public_key().to_string()).unwrap();

    let staking_pool = deploy!(
      contract: StakingContractContract,
      contract_id: "staking_pool",
      bytes: &STAKING_POOL_BYTES,
      signer_account: owner,
      deposit: 500 * NEAR,
      gas: 25*TGAS,
      init_method: new(owner.account_id(), key, RewardFeeFraction{ denominator: 1, numerator: 1 } )
    );
...
#[test]
fn simtest() {
    let (divpool_contract, master_account, testnet, owner, treasury, operator) =
        init_simulator_and_contract(to_yocto("1000"), "me");
    ...
    let transaction = master_account.create_transaction(
        ...
        [
            "sp1",
            ".",
            &divpool_contract.user_account.account_id(),
            ".",
            "testnet",
        ]
        .concat(),
    ...
    let res = transaction.transfer(to_yocto("10")).submit();
