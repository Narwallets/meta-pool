...
use near_sdk::{
    is_promise_success, assert_one_yocto, env, ext_contract, log, near_bindgen, AccountId, Balance, EpochHeight, Gas,
    Promise, PromiseOrValue, PanicOnDefault
};
...
pub trait FungibleToken {
    ...
    fn ft_transfer(&mut self, user_account: AccountId, token_amount:U128String, msg:Option<String>);
}
...
impl Contract {
    ...
    pub fn new(
        ...
        assert!(initial_price_e3.0<=final_price_e3.0,"final price must be >= initial price");
        assert!(closes_at_epoch==0 || opens_at_epoch==0 || closes_at_epoch>opens_at_epoch, "closes_at_epoch must be >= opens_at_epoch");
    ...
    pub fn open(&self) -> Promise {
        ...
        if !self.sell_only {
            ...
            assert!(env::account_balance() >= self.total_near + ONE_NEAR,"sell/buy mode, not enough NEAR balance");
        }
    ...
    pub fn after_ft_balance(&mut self, #[callback] token_balance: U128String ) {
        ...
        if is_promise_success() {
            ...
            assert!(token_balance.0 != self.total_tokens, "Incorrect tokens at account {}: {}. Expected exactly {}", 
                env::current_account_id(), self.token_contract, self.total_tokens);
    ...
    pub fn can_operate(&self) -> bool {
        ...
        if self.opens_at_epoch != 0 && env::epoch_height() < self.opens_at_epoch { return false }
        if self.closes_at_epoch != 0 && env::epoch_height() >= self.closes_at_epoch { return false }
        return true;
    }
    ...
    pub fn buy(&mut self) -> Promise {
        ...
        assert!( attached_near >= self.min_amount_near, "min amount is {}", self.min_amount_near );
        assert!( attached_near <= self.total_near - self.near_received , "max amount is {} NEAR", self.total_near - self.near_received );
        
        //compute token amount with price computed *after* we sell them (buying all pays the higher price)
        let token_amount =
        if self.initial_price_e3 == self.final_price_e3 {
            //easy mode
            ...
            let price_after_e3 = self.initial_price_e3 + proportional(delta_price, near_after, self.total_near);
            proportional(attached_near, 1000, price_after_e3)
        };

        //transfer tokens to user
        ext_ft_contract::ft_transfer(user_account.clone(), token_amount.into(), None,
            //promise params:
            &self.token_contract, //call token contract
    ...
    //prev fn continues here
    #[private] 
    pub fn after_buy(&mut self, user_account:AccountId, near_amount:u128, token_amount:u128) -> U128String {
        if is_promise_success() {
            //transfer was ok
    ...
    //-------------------//-------------------
    pub fn ft_on_transfer(&mut self, sender_id: AccountId, amount:U128String, _msg:String) -> PromiseOrValue<U128> {
        ...
        assert!( token_amount >= self.min_amount_token, "min amount is {} tokens", self.min_amount_token );
        assert!( token_amount <= self.tokens_left , "max amount is {} tokens", self.tokens_left );
        ...
        if self.initial_price_e3 == self.final_price_e3 {
            ...
            let price_after_e3 = self.initial_price_e3 + proportional(delta_price, token_after, self.total_tokens);
            proportional(token_amount, price_after_e3, 1000)
        };

        //transfer NEAR to the user
        Promise::new(sender_id).transfer(near_amount)
        .then(ext_self_callback::after_sell(
            near_amount, token_amount,
            //promise params:
            &env::current_account_id(),//callback
            NO_DEPOSIT, //attached native NEAR amount
            GAS_FOR_AFTER_BUY,
