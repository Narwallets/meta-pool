...
use near_sdk::{
    env, near_bindgen, AccountId, Balance, Gas,
...
impl FungibleTokenCore for MetaPool {
    ...
    #[payable]
    fn ft_transfer(&mut self, receiver_id: ValidAccountId, amount: U128, #[allow(unused)] memo: Option<String>) {
        assert_one_yocto();        
        self.internal_multifuntok_transfer(&env::predecessor_account_id(), &receiver_id.into(), STNEAR, amount.0);
    }
    ...
    fn ft_total_supply(&self) -> U128 {
        self.total_for_staking.into()
    }

    fn ft_balance_of(&self, account_id: ValidAccountId) -> U128 {
        let acc = self.internal_get_account(&account_id.into());
        return self.amount_from_stake_shares(acc.stake_shares).into()
    }
}
