...
impl Simulation {
  ...
  pub fn new() -> Self {
    ...
    for n in 0..=3 {
      ...
      let sp_contract =deploy_simulated_staking_pool(&master_account, acc_id , &owner.account_id());
      //call(&owner,&sp_contract,"pause_staking","{}",0,10*TGAS);
      sp.push( sp_contract );
      //-- register in the staking pool
      call!(owner, metapool.set_staking_pool(acc_id.clone(),weight_basis_points_vec[n]*100), gas=25*TGAS);
    }
  ...
  pub fn show_sps_staked_balances(&self){
    println!("--SPs balance");
    for n in 0..=3 { self.show_sp_staked_balance(n) }
    println!("--------------");
  }
  ...
  pub fn show_account_info(&self, acc:&str) -> Value {
    ...
    return near_sdk::serde_json::from_str(std::str::from_utf8(&result.unwrap()).unwrap()).unwrap();
  }
