...
//-----------
pub fn step_random_action( sim:&Simulation, acc:&UserAccount, action:Action, amount_near: u64, pre:&State) -> Result<StateAndDiff,String> {
  
  println!("step_random_action {:?} {}",action,amount_near);

  return match action {
    Action::Stake => step_call(&sim, &acc, "deposit_and_stake", json!({}), 50*TGAS, amount_near as u128 *NEAR, &pre),
    Action::AddLiquidity => step_call(&sim, &acc, "nslp_add_liquidity", json!({}), 200*TGAS, amount_near as u128 *NEAR, &pre),
    Action::RemoveLiquidity => step_call(&sim, &acc, "nslp_remove_liquidity", json!({"amount": ntoU128(amount_near)}), 200*TGAS, NO_DEPOSIT, &pre),
    Action::DelayedUnstake => step_call(&sim, &acc, "unstake", json!({"amount": ntoU128(amount_near)}), 100*TGAS, NO_DEPOSIT, &pre),
    Action::DUWithdraw => step_call(&sim, &acc, "withdraw", json!({"amount": ntoU128(amount_near)}), 50*TGAS, NO_DEPOSIT, &pre),
    Action::LiquidUnstake => step_call(&sim, &acc, "liquid_unstake", json!({"stnear_to_burn": ntoU128(amount_near), "min_expected_near": ntoU128(amount_near*95/100)}), 50*TGAS, NO_DEPOSIT, &pre),
...
fn simulation_fuzzy() {
  ...
  for seed in START_SEED..END_SEED {
    ...
    let mut initial_commands = step_call(&sim, &users[0], "nslp_add_liquidity", json!({}), 200*TGAS, amount_add_liq, &pre).unwrap();
    //everyone stakes
    for n in 0..COUNT_USERS {
      initial_commands = step_call(&sim, &users[n], "deposit_and_stake", json!({}), 200*TGAS, amount_stake, &initial_commands.state).unwrap()
    }
