...
pub fn bot_distributes(sim:&Simulation, start:&State) -> Result<StateAndDiff,String> {
  ...
  while more_work {
    ...
    match step_call(sim, &sim.operator, "distribute_staking", json!({}), 150*TGAS, NO_DEPOSIT, &state) {
      Err(x) => return Err(x),
      Ok(data) => {
        state = data.state;
  ...
  return Ok(StateAndDiff { state, diff, res:None });
}
...
pub fn bot_ping_rewards(sim:&Simulation, start:&State) -> Result<StateAndDiff,String> {
  ...
  for inx in 0..state.sps.len() {
    ...
    if (staked > 0 || unstaked > 0) && &pool["last_asked_rewards_epoch_height"] != state.epoch {
      ...
      println!("meta.DISTR");
      match step_call(sim, &sim.operator, "distribute_rewards", json!({"sp_inx":inx}), 200*TGAS, NO_DEPOSIT, &state) { 
        Err(x) => return Err(x),
  ...
  for inx in 0..state.sps.len() {
    ...
    if unstaked > 0 && &pool["unstaked_requested_epoch_height"] != "0" {
      ...
      if when<=now {
        ...
        match step_call(sim, &sim.operator, "retrieve_funds_from_a_pool", json!({"inx":inx}), 200*TGAS, NO_DEPOSIT, &state) {
          Err(x) => return Err(x),
          Ok(data) => state = data.state
        }
      }
...
//-----------
pub fn step_call( sim:&Simulation, acc:&UserAccount, method:&str, args:Value, gas:u64, attached_near: u128, pre:&State) -> Result<StateAndDiff, String> {
